...
export default function FeedbackComponent({ docId, feedback, setFeedback, docsId }) {
  ...
  useEffect(() => {
    const fetchFeedback = async () => {
      const payload = { action: "Get_Feedback", doc_id: docsId };
      try {
  ...
  if (isLoading)
    return <CsavvyContentLoader loaderText={"Loading Interview Results"}/>;
  ...
  return (
    <div className="dashboard-container">
      ...
      <main className="interview-feedback-wrapper">
        ...
        {/* <button className="reportDownload bottombutton" onClick={()=>{setIsLoading(true);setTimeout(()=>{setIsLoading(false);navigate("/aspire-quest")},1000);}} >
